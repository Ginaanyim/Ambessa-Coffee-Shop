...
$page = file_get_contents('../cart_view.html');
...
if (preg_match('/<template id="cart-item-template">(.*?)<\/template>/is', $page, $matches)) {
    ...
    // Detta förhindrar att den råa mallkoden visas i webbläsaren.
    $page = preg_replace('/<template id="cart-item-template">.*?<\/template>/is', '', $page);
}
$cartItemsHtml = '';
$totalPrice = 0;
...
if (empty($_SESSION['cart'])) {
    ...
    // Loppa genom varje produkt som finns i kundvagnen   
    if ($result && $result->num_rows > 0) {
        while ($product = $result->fetch_assoc()) {
            ...
            // Skapar sökväg till produktbilden (rot-relativ sökväg är bäst i HTML)
        ...
        $totalText = "Total: " . number_format($totalPrice, 2) . " SEK";
        $checkoutText = "<a href='/php/checkout.php' class='button'>Checkout</a>"; 
    } else {
